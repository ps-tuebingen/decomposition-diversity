...
import qualified RefuncI
import qualified RefuncII
...
new_fun_bods p tn =
  List.map (\x -> (,) (Datatypes.fst x)
    (RefuncII.refunctionalize_expr tn (Datatypes.snd x)))
  ...
  List.filter (\x ->
    ...
    (List.map (\x -> (,) (Datatypes.fst x)
      (List.map (\y -> (,) (Datatypes.fst y)
        (RefuncII.refunctionalize_expr tn (Datatypes.snd y)))
...
switch_indices_aux p sn n tn e =
  RefuncII.refunctionalize_expr tn
...
new_gfun_bods_g p tn =
  Datatypes.app
    (List.map (\ctor -> (,) (Names.unscope (Datatypes.fst ctor))
      ...
      (List.filter (RefuncI.gfunsigs_filterfun_g tn)
          ...
          (localize_aux
            (List.filter
              ...
              (List.flat_map (\x ->
                List.map (\y -> (,) (Datatypes.fst x) y) (Datatypes.snd x))
                (ProgramDef.program_cfun_bods_l p)))))))
      (List.filter (RefuncI.gfunsigs_filterfun_l tn)
        (Skeleton.skeleton_ctors (ProgramDef.program_skeleton p))))
    (List.map (\x -> (,) (Datatypes.fst x)
      (List.map (\y -> (,) (Datatypes.fst y)
        (RefuncII.refunctionalize_expr tn (Datatypes.snd y)))
...
refunctionalize_program :: ProgramDef.Coq_program -> Names.TypeName ->
                           ProgramDef.Coq_program
refunctionalize_program p tn =
  ProgramDef.Coq_mkProgram (RefuncI.refunctionalize_to_skeleton p tn)
