module CtorizeIII where

import qualified Prelude
import qualified AST
import qualified BodyTypeDefs
import qualified CtorizeI
import qualified CtorizeII
...
new_fun_bods p tn =
  List.map (\x -> (,) (Datatypes.fst x)
    (CtorizeII.constructorize_expr tn (Datatypes.snd x)))
...
switch_indices_aux p sn n tn e =
  CtorizeII.constructorize_expr tn
...
new_cfun_bods_g p tn =
  Datatypes.app
    (List.map (\dtor -> (,)
      ...
      (List.filter (CtorizeI.cfunsigs_filterfun_g tn)
        (Skeleton.skeleton_dtors (ProgramDef.program_skeleton p))))
    (List.map (\x -> (,) (Datatypes.fst x)
      (List.map (\y -> (,) (Datatypes.fst y)
        (CtorizeII.constructorize_expr tn (Datatypes.snd y)))
          ...
          (localize_aux
            (List.filter
              ...
              (List.flat_map (\x ->
                List.map (\y -> (,) (Datatypes.fst x) y) (Datatypes.snd x))
                (ProgramDef.program_gfun_bods_l p)))))))
      (List.filter (CtorizeI.cfunsigs_filterfun_l tn)
        (Skeleton.skeleton_dtors (ProgramDef.program_skeleton p))))
    (List.map (\x -> (,) (Datatypes.fst x)
      (List.map (\y -> (,) (Datatypes.fst y)
        (CtorizeII.constructorize_expr tn (Datatypes.snd y)))
...
constructorize_program :: ProgramDef.Coq_program -> Names.TypeName ->
                          ProgramDef.Coq_program
constructorize_program p tn =
  ProgramDef.Coq_mkProgram (CtorizeI.constructorize_to_skeleton p tn)
